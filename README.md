...
| `gitt list` | List worktrees for the repo containing the current working directory. Add `--global` / `-g` to list every worktree the daemon knows about across all repos (the output gains a `REPO` column to disambiguate). Errors if cwd is not inside any git repo, unless `--global` is set. **Requires daemon** |
