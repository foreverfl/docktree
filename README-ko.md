...
| `gitt off` | 데몬 종료 |
| `gitt init [dir]` | `dir` (기본값: 현재 디렉토리)에 bare 레이아웃 레포 초기화. `<dir>/.bare` (bare repo) 생성 → `<dir>/.git` 포인터 파일 작성 → `<dir>/.worktrees/<branch>` 에 빈 orphan worktree 추가. `-b`/`--initial-branch <name>` 으로 초기 브랜치 지정 가능 (기본값: `init.defaultBranch` 설정값, 없으면 `main`). git 2.42+ 필요. `.bare` 또는 `.git` 이 이미 존재하면 거부. 종료 시 데몬 등록 시도 (데몬이 꺼져 있으면 경고). `gitt clone` 의 리모트 없는 버전. |
...
| `gitt rename <old> <new>` | 브랜치와 worktree 폴더를 한 번에 rename. `<repo>/.worktrees/<old>` → `<repo>/.worktrees/<new>` 이동 + 브랜치 rename + 데몬 레코드 갱신을 묶어서 처리. `<old>` 또는 `<new>` 가 `[branches].protected` 에 있으면 거부 (`<new>` 체크는 `gitt rename feat/foo main` 같은 정책 우회를 차단함). **데몬 필요** |
| `gitt list` | 현재 작업 디렉토리가 속한 레포의 worktree 목록을 출력. 출력 컬럼: `BRANCH  PROTECTED  STATUS  PATH`. `PROTECTED` 는 `[branches].protected` 에 있는 브랜치면 `*`, 없으면 공백. `--global` / `-g` 를 붙이면 데몬이 알고 있는 모든 레포의 worktree 를 출력하며 앞에 `REPO` 컬럼이 추가됨. cwd 가 git 레포 안이 아니면 에러로 끊고 `--global` 사용을 안내함. **데몬 필요** |
...
- `protected` — 여기에 나열된 브랜치 이름은 `gitt rename` (소스·대상 모두) 에서 거부됨. 대소문자 구별 정확 일치 (`main` 은 `Main` 을 보호하지 않음). 와일드카드·정규식 미지원; 각 이름을 명시적으로 나열할 것. 데몬이 시작할 때 이 목록을 읽으므로 파일 편집 후 `gitt off && gitt on` 을 실행해야 변경이 반영됨.

> **참고:** `copy`, `symlink`, `ignore` 목록은 아직 `gitt add` 또는 `gitt clone` 에서
> 실제로 사용되지 않음. `[branches].protected` 목록은 현재 `gitt rename` 에 적용됨;
> `gitt remove` 적용은 예정되어 있으나 아직 구현되지 않음.
